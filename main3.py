...
for i in range(100,150):
    old_betas[i-100].append(layer_3_dataset[i][1] + [layer_3_dataset[i][2]])
    old_gammas[i-100].append(layer_3_dataset[i][3] + [layer_3_dataset[i][4]])



graph_dataset3_layer4 = [[] for _ in range(50)]
cntr = 0

for num in range(100,150):
    minimum_energy = 0
    graph = graph_instances[num]
    nelder_old_betas = old_betas[num-100][0]
    nelder_old_gammas = old_gammas[num-100][0]
    ...
    graph_dataset3_layer4[cntr].append(minimum_energy)
    graph_dataset3_layer4[cntr].append(old_betas[num-100][0])
    graph_dataset3_layer4[cntr].append(optimal_beta)
    graph_dataset3_layer4[cntr].append(old_gammas[num-100][0])
    graph_dataset3_layer4[cntr].append(optimal_gamma)
    cntr += 1

np.save('dataset_3_layer4.npy', np.array(graph_dataset3_layer4))
