...
class QAOA():
    def __init__(self, betas, gammas, number_of_qubits, layers, w, graph):
        ...
        self.counts = execute(self.qaoa, Aer.get_backend('qasm_simulator')).result().get_counts()
        self.max_value = max(self.counts, key=self.counts.get)
